...
class MsImageDis(nn.Module):
    '''
    论文中对判别器的描述：D模仿现在流行的多尺度缩放 PatchGAN
    采用不同尺寸的图像进行输入，64*32， 128*64， 256*128
    当D更新到稳定的时候，应用梯度惩罚
    '''
    ...
    def forward(self, x):
        ...
        # 这里一共输出3个元素，分别大小为[batch_size, 1, 64, 32], [batch_size, 1, 32, 16], [batch_size, 1, 16, 8]
        return outputs

    def calc_dis_loss(self, model, input_fake, input_real):        # 该损失是为了训练D
        '''
        判别器不仅要认出fake data，还要认出true data。所以 输入包含fake data 和 true data
        '''
        '''
        :param model:       MsImageDis 本身
        :param input_fake:  fake data，也就是合成的图片
        :param input_real:  true data，训练集里真实的图片
        :return:
        '''
        # calculate the loss to train
        input_real.requires_grad_()
        outs0 = model.forward(input_fake)    # fake data 经过模型的输出
        outs1 = model.forward(input_real)    # true data 经过模型的输出
        loss = 0
        reg = 0
        Drift = 0.001
        LAMBDA = self.LAMBDA
    ...
    def calc_gen_loss(self, model, input_fake):
        '''
        大家可能会有这样的疑问，为什么生成器生成图片之后要在判别器里面去计算损失

        '''

        '''
        :param model:       MsImageDis  本身
        :param input_fake:  fake data
        :return:
        '''
        ...
        for it, (out0) in enumerate(outs0):
            if self.gan_type == 'lsgan':
                ...
                assert 0, "Unsupported GAN type: {}".format(self.gan_type)
        # 这是因为 GAN网络一般有两个模块：生成器和判别器，在生成模块计算损失的时候，是不需要真实图片的，直接将生成器生成的图片输入到
        # 判别器，由判别器判别该图片是真是假，如果是假的，则生成器继续优化。所以生成器损失计算函数还只有一个，只需要一张假的图片
        # 就可以计算损失了。
        return loss
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params, fp16):
        ...
        dim = params['dim']                   # 生成器最后一层filters的数目，默认为16
        n_downsample = params['n_downsample'] # 进行编码下采样的层数，默认设置为2
        n_res = params['n_res']               # 跳跃链接的层数
        activ = params['activ']               # 激活函数，默认为relu
        pad_type = params['pad_type']         # 填充类型
        mlp_dim = params['mlp_dim']           # 多层感知机层的维度，默认为512 filters
        mlp_norm = params['mlp_norm']         # 全连接层正则化的类型，默认为None
        id_dim = params['id_dim']             # Ea的维度，默认为2048
        which_dec = params['dec']             # basic
        dropout = params['dropout']           # dropout
        tanh = params['tanh']                 # 最后一层的激活函数
        non_local = params['non_local']

        # content encoder
        '''
        图片经过（ContentEncoder）Es编码成 st，
        Es由四个卷积层和4个跳跃连接块组成
        input_dim为1，指通道数，Es的输入图片为灰度图
        '''
        ...
        self.output_dim = self.enc_content.output_dim   # 获得解码器输出的特征向量的维度， output_dim = 128
        if which_dec =='basic':        
        ...
        self.apply(weights_init(params['init']))   # 权重初始化

    def encode(self, images):                      # 论文中Es
        # encode an image to its content and style codes
        content = self.enc_content(images)
        return content

    def decode(self, content, ID):                 # 论文中decoder
        # decode style codes to an image
        '''
        :param content:   Es生成的st code
        :param ID:        Ea生成的ap code    ID shape: [4, 8192]
        :return:          合成的图片         shape:[batch_size, 3, 256, 128]
        '''
        ID1 = ID[:,:2048]            # ID1 shape: [4, 2048]
        ID2 = ID[:,2048:4096]
        ID3 = ID[:,4096:6144]
        ID4 = ID[:,6144:]

        # adain_param shape [batch_size, 1024] = [batch_size, 256 * 4]
        # 把ap code分成四个部分进行全连接，得到4个256维 再连接起来
        adain_params_w = torch.cat( (self.mlp_w1(ID1), self.mlp_w2(ID2), self.mlp_w3(ID3), self.mlp_w4(ID4)), 1)  # shape: [4, 1024]
    ...
    def assign_adain_params(self, adain_params_w, adain_params_b, model):
        ...
        dim = self.output_dim     # 128
        for m in model.modules(): # model 包含4个卷积层和4个跳跃连接层
            if m.__class__.__name__ == "AdaptiveInstanceNorm2d":
                mean = adain_params_b[:,:dim].contiguous()
                std = adain_params_w[:,:dim].contiguous()
                # tensor变量调用contiguous()函数会使tensor变量再内存中的存储变得连续
                # view只能用在contiguous的variable上，如果在view之前用了transpose，permutedeng，
                # 需要用contiguous来返回一个contiguous copy
                m.bias = mean.view(-1)    # 相当于reshape， 把所有维度都乘起来变成一维  batch_size * dim
                m.weight = std.view(-1)
                if adain_params_w.size(1)>dim :  #Pop the parameters
                    adain_params_b = adain_params_b[:,dim:]
                    adain_params_w = adain_params_w[:,dim:]

    def get_num_adain_params(self, model):       # 返回AdaIN模型中的全部参数个数
...
class ContentEncoder(nn.Module):     # 论文中Es 包含4个卷积层和4个跳跃连接块
    '''
    input： [batch_size, 1, 256, 128]
    output：[batch_size, 128, 64, 32]
...
class ContentEncoder_ImageNet(nn.Module):    # 使用resnet50的 Es
...
class Decoder(nn.Module):    # 4个连接跳跃块和4个卷积层（dropout、NonelocalBlock）
    '''
    input： [batch_size, 128, 64, 32]
    output：[batch_size, 3, 256, 128]
    '''
    ...
    def __init__(self, n_upsample, n_res, dim, output_dim, dropout=0, res_norm='adain', activ='relu', pad_type='zero', res_type='basic', non_local=False, fp16 = False):
        ...
        if non_local>0:
            ...
            print('use non-local!')
        for i in range(n_upsample):   # 两层的上采样
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, dim, n_blk, norm='in', activ='relu'):
        ...
        :param input_dim:  2048
        :param output_dim:  256
        :param dim:         512
        :param n_blk:
        :param norm:
        :param activ:
...
class NonlocalBlock(nn.Module):     # self_attention, 注意力可微，更关注区域和通道
    def __init__(self, in_dim, norm='in'):
        '''
        self attention 经过三个卷积层得到3个分支：query，key，value
        第一步：将query和每个key进行相似度计算得到权重，这里使用的是点积
        第二步：softmax 归一化权重
        第三步：将归一化权重和key相应的value进行加权求和，得到最后的attention
        '''
    ...
    def forward(self,x):
        ...
        energy =  torch.bmm(proj_query, proj_key) # transpose check    query 和 key 进行点积
...
class ASPP(nn.Module):    # ASPP ： 空洞空间卷积池化金字塔
    '''
    对输入的图片以不同采样率rate的空洞卷积进行采样
    其中空洞卷积的rate的意思是在普通卷积的基础上，相邻权重之间的间隔为rate-1， 普通卷积的rate默认为1
    然后将各个每个卷积层网络输出拼接起来，再进行卷积操作
    '''
    # ASPP (a)
    def __init__(self, dim, norm='in', activation='relu', pad_type='zero'):
        ...
        self.conv6 += [Conv2dBlock(dim_part,dim_part, 3, 1, 3, norm=norm, activation='none', pad_type=pad_type, dilation=3)]    # 采样率为3
        ...
        self.conv12 += [Conv2dBlock(dim_part,dim_part, 3, 1, 6, norm=norm, activation='none', pad_type=pad_type, dilation=6)]   # 采样率为6
        ...
        self.conv18 += [Conv2dBlock(dim_part,dim_part, 3, 1, 9, norm=norm, activation='none', pad_type=pad_type, dilation=9)]   # 采样率为9
        self.conv18 = nn.Sequential(*self.conv18)

        self.fuse = Conv2dBlock(4*dim_part,2*dim, 1, 1, 0, norm=norm, activation='none', pad_type=pad_type)      # 最后用卷积层进行融合
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
        ...
        if norm == 'sn':
            self.conv = SpectralNorm(nn.Conv2d(input_dim, output_dim, kernel_size, stride, dilation=dilation, bias=self.use_bias))   # 矩阵的最大奇异值
        #  矩阵除以它的spectral norm（最大特征值的开根号）可以使其具有1 - lipschitz continuity， lipschitz 限制了函数变化的剧烈程度即函数的梯度
        #  神经网络中只需要保证参数矩阵具有1 - lipschitz continuity，然而这种方式计算量很大，使用Power Iteration近似计算矩阵最大特征值
        else:
...
class Series2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
        ...
        '''
        在一个channel方向做归一化，算H*W的均值，用在风格化迁移，因为在图像风格化中，生成结果主要依赖于某个图像实例，
        所以对整个batch归一化不适合应用在图像风格化中，因而对H*W做归一化。这样做可以加速模型收敛，并且保持每个图象实例之间的独立。
        '''
...
class Parallel2dBlock(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(self.pad(x)) + self.norm(x)   # 卷积之后的图像加上归一化  再实例归一化
        x = self.instance_norm(x)
        if self.activation:
            x = self.activation(x)
        return x
...
##################################################################################
class AdaptiveInstanceNorm2d(nn.Module):    # 对H*W进行归一化
    ...
    def forward(self, x):
        ...
        # Apply instance norm
        x_reshaped = x.contiguous().view(1, b * c, *x.size()[2:])    # [ 1, batch_size*channel, H*W]
        out = F.batch_norm(
            x_reshaped, running_mean, running_var, self.weight, self.bias,
            True, self.momentum, self.eps)    # batch_norm 是在batch上对CNW做归一化
...
class LayerNorm(nn.Module):   # 在一个batch上计算均值和方差，对batch敏感
    def __init__(self, num_features, eps=1e-5, affine=True, fp16=False):
        ...
        if self.affine:
            ...
            # nn.Parameter :将一个不可训练的类型Tensor转化为可以训练的类型parameter并将这恶parameter绑定到module里面
            # net.parameter()中就有这个绑定的parameter，所以在参数优化的时候可以进行优化
            self.beta = nn.Parameter(torch.zeros(num_features))
    def forward(self, x):
        shape = [-1] + [1] * (x.dim() - 1)      # shape: [-1, 1 * x.dim() - 1]
        if x.type() == 'torch.cuda.HalfTensor': # For Safety    16 bit 半精度浮点
            mean = x.view(-1).float().mean().view(*shape)    # 均值
            std = x.view(-1).float().std().view(*shape)      # 方差
            mean = mean.half()
            std = std.half()
        else:
