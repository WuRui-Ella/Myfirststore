...
train_loader_a, train_loader_b, test_loader_a, test_loader_b = get_all_data_loaders(config)    # function from utils.py
...
while True:
    for it, ((images_a,labels_a, pos_a),  (images_b, labels_b, pos_b)) in enumerate(zip(train_loader_a, train_loader_b)):
        ...
        with Timer("Elapsed time in update: %f"):
            ...
            torch.cuda.synchronize()   # 等着GPU把所有工作都做完
        ...
        # Write images
        if (iterations + 1) % config['image_save_iter'] == 0:     # 可以看到在训练中生成器生成的重构图片或者合成图片是什么样子的
            with torch.no_grad():
                if num_gpu>1:
                    test_image_outputs = trainer.module.sample(test_display_images_a, test_display_images_b)
                    # sample  : 输入：两张图片x_a,x_b， 输出：x_a, x_a_recon, x_aba, x_ab1, x_b, x_b_recon, x_bab, x_ba1
                else:
