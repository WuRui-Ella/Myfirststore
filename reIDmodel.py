...
######################################################################
def weights_init_kaiming(m):      # 参数初始化
...
def weights_init_classifier(m):  # 分类器参数初始化
    classname = m.__class__.__name__
    if classname.find('Linear') != -1:
        init.normal_(m.weight.data, std=0.001)
        init.constant_(m.bias.data, 0.0)

def fix_bn(m):                   # 固定 batch_norm
    classname = m.__class__.__name__
...
class ft_netAB(nn.Module):
    '''
    论文中关于Ea的描述：使用的是基于ResNet50预训练的ImageNet模型，
    并且移除了全局平均池化层和全连接层，然后添加了一个合适的max pooling层去输出ap code(2048 * 4 * 1)
    ...
    def __init__(self, class_num, norm=False, stride=2, droprate=0.5, pool='avg'):
        ...
        self.part = 4
        if pool=='max':
            model_ft.partpool = nn.AdaptiveMaxPool2d((self.part,1))    # 参数为H*W输出维度
            model_ft.avgpool = nn.AdaptiveMaxPool2d((1,1))
        else:
            model_ft.partpool = nn.AdaptiveAvgPool2d((self.part,1))
            model_ft.avgpool = nn.AdaptiveAvgPool2d((1,1))
    ...
    def forward(self, x):
        ...
        # 此时开始进行分离，获得 [衣服+鞋子+包+其他等]特征
        # f [batch_size, 2048, 4, 1]
        f = self.model.partpool(x)
        f = f.view(f.size(0),f.size(1)*self.part)   # resize-> f: [batch_size, 2048, 4]
        f = f.detach() # no gradient，              # f这个值以后不再计算梯度
        ...
        x1 = self.classifier1(x)    # prim  [batch_size, 751]
        x2 = self.classifier2(x)    # fine  [batch_size, 751]
        x=[]
        x.append(x1)
        x.append(x2)
        return f, x
