...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'train_all'), batch_size, True,                 # data_root： 已准备好的数据集
...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
    ...
    dataset = ImageFilelist(root, file_list, transform=transform)    # from data.py
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=128, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),                                                  # to tensor
                      transforms.Normalize((0.485, 0.456, 0.406),
                                           (0.229, 0.224, 0.225))]                            # normalization
    transform_list = [transforms.RandomCrop((height, width))] + transform_list if crop else transform_list    # 随机裁剪
    transform_list = [transforms.Pad(10, padding_mode='edge')] + transform_list if train else transform_list  # 图片填充
    transform_list = [transforms.Resize((height,width), interpolation=3)] + transform_list if new_size is not None else transform_list
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list        # 图片随机翻转
    transform = transforms.Compose(transform_list)
    dataset = ReIDFolder(input_folder, transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers)
    return loader            # 返回：样本数据， 样本数据的label， 样本数据的正例数据（因为是同一个label）
